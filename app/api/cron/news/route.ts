...
function isAuthorized(request: NextRequest): boolean {
  ...
  // In local development or if CRON_SECRET is not configured yet, allow manual test requests
  if (!cronSecret) {
    return true;
  }
  ...
  if (process.env.NODE_ENV !== "production" && url.searchParams.get("manual") === "true") {
    return true;
  }
...
async function handleScraperRequest(request: NextRequest) {
  if (!isAuthorized(request)) {
    return NextResponse.json(
      {
        error: "Unauthorized",
        message: "Valid CRON_SECRET authorization header or ?secret= query parameter required.",
      },
      { status: 401 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: (error as Error).message || "Internal server error occurred during scraper run.",
      },
      { status: 500 }
    );
  }
